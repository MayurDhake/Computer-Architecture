	     ...
	     md_addr_t *repl_addr)	/* for address of replaced block */
{
  ...
  struct cache_blk_t *blk, *repl, *location1;
  ...
  cp->misses++;
	...
	if(cp->policy==TLFULRU)	  // Toggle between LFU and LRU
	{
		original_policy_TLFULRU=1010;	//Identifier
	}
	
	if(original_policy_TLFULRU==1010)	//Miss for TLFULRU replacement policy 
	{		
	
	if((cp->misses % cp->assoc)==0)		//After every "cp->assoc" number of misses, we remove the block with highest frequency	
		cp->policy=LRU;
	else 			
		cp->policy=LFU;
	}
  ...
  case Random:
    ...
    break;
  case MRU:			// Miss for Most Recently Used
  {
	  repl=cp->sets[set].way_head;
	  update_way_list(&cp->sets[set], repl, Head);
      break;
  }
  case LFU:			/*Miss for Least Frequently Used case*/
  {
			
		blk = cp->sets[set].way_tail;
		minimum = blk->way_frequency;
		location1 = blk;
	
	
	  for (blk=cp->sets[set].way_head; blk!=NULL; blk = blk->way_prev)
	  {
		  if (blk->way_frequency < minimum)
		  {
			  minimum = blk->way_frequency;
			  location1=blk;
		  }
  ...
  case MFU:		/*Miss for Most Frequently Used case*/
  {
	  blk=cp->sets[set].way_tail;
	  location1=blk;
	  maximum=blk->way_frequency;
	  for(blk=cp->sets[set].way_tail; blk != NULL; blk=blk->way_prev)
	  {
		  if(blk->way_frequency > maximum)
		  {
			  maximum=blk->way_frequency;
			  location1=blk;
		  }
 ...
 cache_hit: /* slow hit handler */
  ...
  if (cp->policy == LFU)
  {
	  blk->way_frequency = blk->way_frequency + 1;
   ...
   /*If TLFULRU replacement*/
  if (cp->policy == TLFULRU)
  {
	  blk->way_frequency = blk->way_frequency + 1;
